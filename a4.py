...
# Jennifer Lin
# October 27,2016
# Consultants Samuel Opoku-Agyemang and Alexander Strandberg 
# explained important concepts that helped me with writing the code.
...
def draw_hex(t, s):
    ...
    #Draw 6 equilateral triangles to make hexagon
    for x in range(0,6):
...
def draw_spiral(w, side, ang, n, sp):
    ...
    #Create turtle on the window
    t = tkturtle.Turtle(w)
    
    #Turtle faces east
    t.heading = 0
    
    #Calls spiral helper fx
...
def draw_spiral_helper(t, side, ang, n, sp):
    ...
    defaultcolor = t.color #Store turtle's default color
    defaultspeed = t.speed #Store turtle's default speed
    
    for x in range (0,n): 
    #Change color of lines red, blue, green
        if x % 3 == 0:
            t.color = 'red'
        elif x % 3 == 1:
            t.color = 'blue'
    ...
    #Draws lines for length of each spiral side
        t.forward((x+1)*side)
        
    #Turn turtle to angle ang
        t.left(ang)
    
    t.color = defaultcolor #Return turtle's default color
    t.speed = defaultspeed #Return turtle's default speed
...
def multi_polygons(w, side, k, n, sp):
    ...
    assert is_valid_iteration(k), report_error('k is not a valid number of'
                                               'iterations',k)
    assert type(n) == int and n >=3, report_error('n is not a valid number'
                                               'of polygon sides',n)
...
def multi_polygons_helper(t, side, k, n, sp):
    ...
    assert is_valid_iteration(k), report_error('k is not a valid number'
                                               'of polygons to draw',k)
    assert type(n) == int and n >=3, report_error('n is not a valid number of'
                                              'polygon sides',n)
    ...
    #Alternate line color between red and green
        if x % 2 == 1:
            t.color = 'red'
            t.speed = sp                 #Speed of turtle
            draw_polygon(t, side, n, sp) #Draws polygon
            t.left(360.0/k)              #Turn turtle after each polygon
        elif x % 2 == 0:
            t.color = 'green'
            t.speed = sp                 #Speed of turtle
            draw_polygon(t, side, n, sp) #Draws polygon
...
def sierpinski(w, side, d):
    ...
    assert is_window(w), report_error('w is not a valid window',w)
    # Added asserts
    assert is_valid_length(side), report_error('side is not a valid side-length'
                                               , side)
...
def sierpinski_helper(p, x, y, side, d):
    ...
    # Samuel Opoku-Agyemang (so373) guided me on how to approach this.
    ...
    #Depth greater than 0 (recursive case)
    if d > 0:
        import math
        h = (side/4.0)*math.sqrt(3.0)
        
        sierpinski_helper(p,x,y,side/2.0,d-1)
        sierpinski_helper(p,x+side/4.0,y+h,side/2.0,d-1)
...
def branches(w, hght, d):
    ...
    assert is_window(w), report_error('w is not a valid window',w)
    #Added asserts
    assert is_valid_length(hght), report_error('hght is not a valid side length'
                                               , hght)
    assert is_valid_depth(d), report_error('d is not a valid depth', d)
...
def branches_helper(t, hght, d):
    ...
    # Alexander Strandberg (ads286) guided me on how to approach this.
    ...
    if d > 0:
        t.forward(hght/2.0)             #Reaches halfway up vertical line
        t.left(90)                      #Turns left to west
        branches_helper(t, hght/2.0, d-1)   #3-tree branch to west
        t.right(90)                     #Turns right to north
        branches_helper(t, hght/2.0, d-1)   #3-tree branch to north
        t.right(90)                     #Turns right to east
        branches_helper(t, hght/2.0, d-1)   #3-tree branch to east
        t.right(90)                     #Turns right to south
        t.forward (hght/2.0)            #Walks back to original position
        t.right(180)
